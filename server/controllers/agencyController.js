...
class AgencyController{
    static registerAgency(req, res, next){
        const {name, email, password, address, city, logoUrl, phoneNumber} = req.body
        Agency.create({name, email, password, address, city, logoUrl, phoneNumber})
        .then(agency => {
    ...
    static getAllAgencies(req, res, next){
        Agency.findAll({attributes: ['id', 'name', 'email', 'password', 'address', 'city', 'logoUrl', 'phoneNumber']})
        .then(agencies => {
    ...
    static updateDataAgency(req, res, next){
        ...
        .then(agecy => {
            if(!agecy){
                ...
                return Agency.update({name, email, password, address, city, logoUrl, phoneNumber}, {where: {id}})
            }
        })
        .then(resp => {
            if(resp[0] === 1){
